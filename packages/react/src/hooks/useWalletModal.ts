...
// Simple global store for modal open state (fallback when outside provider)
...
/**
 * Hook for wallet modal state management.
 * Provides complete modal state management, including source tracking,
 ...
 * @example
 * ```tsx
 * import { useWalletModal } from '@btc-connect/react';
 *
 ...
 *       </button>
 *
 *       <button onClick={open}>Open (default source)</button>
...
export function useWalletModal(): UseWalletModalReturn {
  // Try to get context, fallback to global store if not available
  ...
  try {
    ...
    // Outside provider, use global store
    ...
    contextModalState = {
      isModalOpen,
      openModal: useCallback(() => modalStore.open(), []),
      closeModal: useCallback(() => modalStore.close(), []),
    };
  }

  const { isModalOpen, openModal, closeModal } = contextModalState;
